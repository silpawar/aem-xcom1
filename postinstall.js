...
// Copy specified files from node_modules/@silpawar to scripts/__dropins__
if (fs.existsSync("node_modules/@silpawar")) {
  fs.readdirSync("node_modules/@silpawar", { withFileTypes: true }).forEach(
    (file) => {
      // Skip if package is not in package.json dependencies / skip devDependencies
      if (!dependencies[`@silpawar/${file.name}`]) {
        return;
      }
      ...
      fs.cpSync(
        path.join("node_modules", "@silpawar", file.name),
        path.join(dropinsDir, file.name),
        {
          recursive: true,
          filter: (src) => !src.endsWith("package.json"),
